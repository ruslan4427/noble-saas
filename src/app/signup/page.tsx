    ...
    <div className="flex gap-2 justify-center" onPaste={handlePaste}>
      {digits.map((d, i) => (
        <input
          ...
          className={`w-11 h-14 text-center text-xl font-bold bg-white/10 border-2 rounded-xl text-white outline-none transition ${
            d ? 'border-[#C9A84C]' : 'border-white/20 focus:border-[#C9A84C]/60'
          }`}
        />
      ))}
    </div>
        ...
        {step === 'credentials' ? (
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-6">
            ...
            <div className="text-center space-y-3">
              <p className="text-white/30 text-xs">Enter the code from the email to continue</p>

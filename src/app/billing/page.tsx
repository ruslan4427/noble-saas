...
const PLANS = [
  {
    ...
    price: 19,
    desc: 'Up to 2 staff',
    features: ['Online booking', 'Up to 2 staff', 'Email notifications'],
  },
  {
    id: 'pro',
    name: 'Pro',
    price: 39,
    desc: 'Up to 5 staff',
    features: ['Everything in Starter', 'SMS reminders', 'Analytics'],
    highlight: true,
  },
  {
    id: 'business',
    ...
    features: ['Everything in Pro', 'Google Calendar', 'Priority support'],
  },
]
  ...
  return (
    <main className="min-h-screen bg-[#0F0A00] text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        {reason === 'canceled' && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 px-4 py-3 rounded-xl mb-8">
            ⚠️ Your subscription was canceled. Reactivate to continue using Noble.
          </div>
        )}
        {success && (
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          {PLANS.map(plan => {
            ...
            return (
              <div key={plan.id} className={`rounded-xl p-6 border flex flex-col ${'highlight' in plan && plan.highlight ? 'border-[#C9A84C] bg-[#C9A84C]/5' : 'border-white/10 bg-white/5'}`}>
                ...
                {isCurrent ? (
                  <div className="w-full text-center py-2 rounded text-sm border border-green-500/30 text-green-400">
                    ✓ Current plan
                  </div>
                ) : (
                  <button
                    ...
                    className={`w-full py-2 rounded font-semibold text-sm transition ${'highlight' in plan && plan.highlight ? 'bg-[#C9A84C] text-black hover:bg-[#e8d08a]' : 'border border-white/20 text-white hover:border-white/40'} disabled:opacity-40`}>
